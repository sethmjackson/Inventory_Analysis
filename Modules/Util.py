...
def stringToDatetime(df: pd.DataFrame, columns: List[str], format=None, inplace=False):
    ...
    if inplace:
        ...
        result = df.copy()
    for c in columns:
        result[c] = pd.to_datetime(result[c], format=format)
...
def toNumericString(df: pd.DataFrame, columns: List[str]):
    for col in columns:
...
def dateToSeason(month: str):
    monthI = int(month)

    if monthI in (12,1,2):
        return 'Winter'
    elif monthI in (3,4,5):
        return 'Spring'
    elif monthI in (6,7,8):
        return 'Summer'
    elif monthI in (9,10,11):
        return 'Fall'
    else:
